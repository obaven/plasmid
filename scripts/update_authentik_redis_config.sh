...
if [ -z "${PLASMIDR_KEY_USERNAME:-}" ] || [ -z "${PLASMIDR_KEY_PASSWORD:-}" ]; then
    echo "ERROR: PLASMIDR_KEY_USERNAME/PASSWORD not set."
    exit 1
fi
...
kubectl patch secret "$CONFIG_SECRET" -n "$APP_NAMESPACE" --type=json -p="[
  {\"op\":\"replace\",\"path\":\"/data/AUTHENTIK_REDIS__USERNAME\",\"value\":\"$(printf '%s' \"$PLASMIDR_KEY_USERNAME\" | base64 -w0)\"},
  {\"op\":\"replace\",\"path\":\"/data/AUTHENTIK_REDIS__PASSWORD\",\"value\":\"$(printf '%s' \"$PLASMIDR_KEY_PASSWORD\" | base64 -w0)\"}
]"
